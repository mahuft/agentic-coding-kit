...
当你让 Agent 理解某个代码库时，工作流不会直接随机读文件，而是先通过高层关系扫描校准一个适中的探索范围：如果问题过大，就先拆成子问题；如果问题过小，就先补入必要上下文。范围稳定后，再确定讲解路径与推荐探索方法，最后按结构化方式探索源码并写出分析报告。
...
② clarify-and-plan（澄清与计划）
  │  通过高层关系探索与一次一问，校准适中的探索范围
  │  生成以讲解主线和推荐探索方法为中心的探索计划
...
| 文件 | 产出阶段 | 说明 |
|------|---------|------|
| `analysis-plan.md` | clarify-and-plan | 结构化探索计划，供下游显式消费当前问题表述、推荐探索方法、讲解主线、问题链、图示计划与范围边界 |
